...
private fun buildAlmanacEntry(rules : List<String>) : (Long) -> Long {
    ...
    return lambda@{ input ->
        val (ruleTarget, ruleSource, range) = parsedRules.lastOrNull { input >= it.second } ?: return@lambda input.also { println("No rule found, returning $it") }

        println("Input $input, found rule $ruleSource $ruleTarget $range")
...
private fun solution1(strings : String) : Long {

    val lines = strings.lines()
    ...
    val alamanac = mapRanges.map { (startIndex, endIndex) ->
...
private operator fun LongRange.minus(other: LongRange) : Collection<LongRange> {
    ...
    val partTwo = (other.endExclusive .. this.last) intersect this
...
private data class Rule(
    ...
    fun applyTo(interval : LongRange) : Pair<LongRange, Collection<LongRange>> {
        ...
        return (mappedTarget to unmappedPart).also { println("        Applying rule $this to $interval yields $it") }
    }
}
...
private fun applyRules(rules : Collection<Rule>, interval : LongRange) : Collection<LongRange> {
    ...
    return buildSet {
        addAll(unmapped)
        addAll(result)
    }.also { println("    Applying all rules to $interval yields $it") }
}

private fun solution2(strings : String) : Long {
    ...
    rulesList.forEach { rules ->
        ...
        println("After step, got $list")
    }

    return list.filter { !it.isEmpty() }.minOf { it.first }
}

//fun main() = println(solution1(example1))

fun main() {
    println("""
        --- Part 1 ---
        on Example: ${solution1(example1)}
        on Input:   ${solution1(input)}
        """)
    println("""
        --- Part 2 ---
        on Example: ${solution2(example2)}
        on Input:   ${solution2(input)}
    """.trimIndent())
}
